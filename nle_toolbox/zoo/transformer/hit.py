...
class HiT(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.iox = nn.Parameter(torch.randn(
            1, 1, n_io, embedding_dim,
        ))
    ...
    def forward(self, input, hx=None):
        ...
        if hx is None:
            hx = repeat(
                torch.zeros_like(self.iox),
                'L () N F -> L B N F', B=n_batch
            )
        ...
        for x in input:
            ...
            x_cls, x_iox, upd_hx, _ = torch.split(x, [
                self.n_cls, self.n_io, self.n_io, n_len,
            ], dim=1)
